...
//! [`ArtifactChunk`] frames are appended at the persisted byte offset — the
//! resume point — so a re-delivered chunk is absorbed without corrupting
//! the file. When the last byte lands, the complete file is verified
//! against the declared SHA-256 and marked `Ready`, or `Failed` on a
//! digest mismatch (§14.3 明确失败).
    ...
    Cursor: CenterCursor,
{
    ...
    /// Consumes one event batch: every record is validated and appended
    /// with the §14.4 dedup and the site association; a record that cannot
    /// be decoded is logged and skipped without failing the batch.
    ...
    /// Consumes one artifact chunk: the bytes are appended at the persisted
    /// resume offset — a re-delivered chunk is absorbed — and when the
    ...
    async fn consume_artifact_chunk(
        ...
        let Some(next) = uploaded.checked_add(received) else {
            return self
                .absorb_frame(
                    site,
                    sequence,
        ...
        // The file write follows the site-side distribution discipline
        // (§7.8): the artifact bytes are touched under a blocking call, and
        // the row advances only after the write succeeds — a failed row
        // update re-writes the same range on the retry.
        let path = self.store.artifact_file_path(artifact_id);
        let data = chunk.data.clone();
        tokio::task::spawn_blocking(move || append_bytes(&path, &data))
            .await
        ...
        if next < artifact.size_bytes() {
            self.store
                .update_artifact(artifact_id, next, ArtifactState::Uploading, now)
                .await
                .map_err(CenterProjectionError::Artifact)?;
        } else {
            self.finalize_artifact(artifact_id, next, now).await?;
        }
        self.advance_cursor(site, SyncStream::Artifact, sequence, now)
            .await
    }
...
/// Appends one chunk to the artifact file; the blocking half of the §7.8
/// file I/O. The artifact directory is created on demand, exactly like the
/// site-side upload flow: the manifest may outlive any file, and the first
/// chunk is the moment the bytes exist.
fn append_bytes(path: &std::path::Path, data: &[u8]) -> Result<(), std::io::Error> {
...
mod tests {
    ...
    -> Result<(), Box<dyn Error>> {
        ...
        // The chunks land at the persisted resume offset.
