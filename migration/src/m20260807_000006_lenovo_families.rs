...
/// Extends the `ck_resources_feature` allow-list with the 0.5.0 Lenovo
/// `SecurityService` family code (`lenovo-security-service`).
///
...
/// Every new resource family code extends the allow-list through a follow-up
...
///
/// The rebuilt tables are byte-identical in shape to the 000001 originals
...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        create_resource_tables_with(manager, FEATURE_ALLOW_LIST_WITH_LENOVO_FAMILY).await
    }

    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        create_resource_tables_with(manager, FEATURE_ALLOW_LIST_BEFORE_LENOVO_FAMILY).await
    }
}

/// The 000003 allow-list (the 000001 list plus the NVIDIA system-config-profile,
/// power-compliance, and managed-entity codes) plus the 0.5.0 Lenovo
/// `SecurityService` code.
const FEATURE_ALLOW_LIST_WITH_LENOVO_FAMILY: &[&str] = &[
    "service-root",
    "systems",
    "chassis",
    "managers",
...
/// The exact 000003 allow-list (the 000001 list plus the three NVIDIA codes),
/// used by `down` to restore the previous constraint shape: the Lenovo family
/// code becomes unparseable again.
const FEATURE_ALLOW_LIST_BEFORE_LENOVO_FAMILY: &[&str] = &[
    "service-root",
    "systems",
    "chassis",
    "managers",
