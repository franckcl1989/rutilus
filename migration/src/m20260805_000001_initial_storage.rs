...
/// W8-D-1 defect's third recurrence surface). `up` runs eleven statements
/// (six `CREATE TABLE`s, five `CREATE INDEX`es) and `down` seven (six
/// `DROP TABLE`s and one `UPDATE` — the `credentials.active_version_id`
/// NULL-out that unblocks the `credential_versions` drop under
/// `ON DELETE RESTRICT`) that `SQLite` would otherwise auto-commit one by
/// one: a crash between them would leave the migration half-applied while it
/// still records as applied, and the retried run would then fail — `up` with
/// "table already exists" (no `IF NOT EXISTS`), `down` with "no such table"
/// forever, blocking the whole rollback chain. `CREATE TABLE`, `CREATE
/// INDEX`, `DROP TABLE`, and `UPDATE` are all legal `SQLite` statements
/// inside a transaction, so the override costs nothing but the crash-resume
/// guarantee — the same discipline the `m20260814_000003` slice (W8-D-1) and
/// the rebuild migrations already follow.
