...
/// W8-D-1 defect's third recurrence surface). `up` runs seven statements
/// (four `CREATE TABLE`s, three `CREATE INDEX`es) and `down` four
/// `DROP TABLE`s that `SQLite` would otherwise auto-commit one by one: a
/// crash between them would leave the migration half-applied while it still
/// records as applied, and the retried run would then fail — `up` with
/// "table already exists" (no `IF NOT EXISTS`), `down` with "no such table"
/// forever, blocking the whole rollback chain. These statements are all legal
