...
/// system-config-profile family code (`nvidia-system-config-profile`).
///
...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        create_resource_tables_with(manager, FEATURE_ALLOW_LIST_WITH_NVIDIA).await
    }

    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        create_resource_tables_with(manager, FEATURE_ALLOW_LIST_BEFORE_NVIDIA).await
    }
}

/// The 000003 allow-list plus the 0.5.0 NVIDIA system-config-profile code.
const FEATURE_ALLOW_LIST_WITH_NVIDIA: &[&str] = &[
    "service-root",
    "systems",
    "chassis",
    "managers",
...
/// The exact 000003 allow-list, used by `down` to restore the previous
/// constraint shape.
const FEATURE_ALLOW_LIST_BEFORE_NVIDIA: &[&str] = &[
    "service-root",
    "systems",
    "chassis",
    "managers",
