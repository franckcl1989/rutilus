...
/// The 000003 allow-list plus the 0.5.0 Lenovo `SecurityService` code: the
/// follow-up migration must accept every code the previous migration accepted
/// and the new family code on top.
const FEATURE_CODES_WITH_LENOVO_FAMILY: [&str; 37] = [
    "service-root",
    "systems",
    "chassis",
    "managers",
...
/// The 000003 allow-list, which `down` must restore exactly (the Lenovo family
/// code becomes unparseable again).
const FEATURE_CODES_BEFORE_LENOVO_FAMILY: [&str; 36] = [
    "service-root",
    "systems",
    "chassis",
    "managers",
...
#[tokio::test]
async fn lenovo_family_migration_extends_the_feature_allow_list() -> Result<(), Box<dyn Error>> {
    ...
    // The follow-up migration accepts every 000003 code plus the new Lenovo
    // family code, and refuses anything else.
    for code in FEATURE_CODES_WITH_LENOVO_FAMILY {
    ...
    assert!(
        seed_resource(
            ...
            endpoint_id,
            "oem-lenovo",
            "/redfish/v1/fixtures/oem-lenovo",
            now,
        )
        .await
        .is_err()
    ...
    // `down` restores the 000003 allow-list: the Lenovo family code is
    // refused again and every original code still works. The rows stored
    // under the new code cannot be represented by the 000003 schema, so they
    // are removed first — exactly what a real downgrade must do with rows
    // that only the newer schema can hold (the rebuild copies every
    ...
    resource::Entity::delete_many()
        ...
        .exec(&database)
        .await?;
    // Down six migrations only: `down(None)` would unwind the whole
    ...
    // failure migration (000012), the audit execute-operation migration
    // (000008), the center tables migration (000009), and the two 0.7.0
    // center site-scoping migrations (000010/000011) registered above it
    // unwind first, so the restore lands on the exact 000003-with-000001
    // allow-list the test asserts.
    Migrator::down(&database, Some(6)).await?;
    assert!(
        seed_resource(
            &database,
            endpoint_id,
            "lenovo-security-service",
            "/redfish/v1/fixtures/downgraded/lenovo-security-service",
            now,
        )
        .await
        .is_err(),
        "lenovo-security-service must be refused after the downgrade"
    );
    for code in FEATURE_CODES_BEFORE_LENOVO_FAMILY {
...
async fn seed_endpoint(
    ...
    endpoint::ActiveModel {
        id: Set(endpoint_id),
        display_name: Set(String::from("Lenovo BMC")),
        created_at: Set(now),
        updated_at: Set(now),
        site_id: Set(None),
