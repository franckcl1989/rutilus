...
-> Result<(), Box<dyn Error>> {
    ...
    resource::Entity::delete_many()
        ...
        .exec(&database)
        .await?;
    // Down six migrations only: `down(None)` would unwind the whole history
    ...
    // shapes, 000006 Lenovo families, 000008 audit execute-operation, and
    // 000009 center tables) unwind first, so the restore lands on the exact
