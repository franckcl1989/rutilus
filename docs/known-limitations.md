...
| 家族 | 表单现状 | 事实来源 |
|---|---|---|
| Telemetry | `CommandFamilyView::ALL` 刻意不含 Telemetry；表单选择器返回 `OperationFormError::FamilyRequired`；界面提示 "The telemetry write form is a later milestone."；已持久化的遥测命令通过 `wire_command_summary` 在卡片中渲染 | `ui/src/lib.rs` 第 5032、6290-6296、11132-11141 行（`CommandFamilyView::ALL` 9 家族 `:5032`、Telemetry 表单拒绝 `:6290-6296`、later-milestone 提示文案串 `:11139`） |
...
| 产品版本号（已统一）+ Git Commit 嵌入 | workspace 版本 = `0.9.0`（生产候选，`rutilus version` 输出），单一版本来源 = 根 `Cargo.toml` `[workspace.package] version`（`Cargo.toml:14`）；`rutilus version` 输出**三行**（`app/src/main.rs:733-737`）：产品版本 / `nv-redfish` 开发基线 / `git commit`——CI 构建经 job 级 `RUTILUS_GIT_COMMIT` 注入（`ci.yml:45-55`，值为 `github.sha`），`GIT_COMMIT` 常量嵌入（`app/src/main.rs:38-40`），本地构建（无该变量）降级为 `dev`（不 spawn git 子进程）；版本/日志格式测试断言由 `env!("CARGO_PKG_VERSION")`、`NV_REDFISH_DEVELOPMENT_BASELINE` 与编译期 `RUTILUS_GIT_COMMIT` 派生（`app/tests/version.rs:27-36`、`app/tests/log_format.rs:23-28`），升级只改一处 | 根 `Cargo.toml:14`；`ci.yml:45-55`；`app/src/main.rs:38-40, 733-737`；`app/tests/version.rs:8-11, 27-36`；`app/tests/log_format.rs:7-10, 23-28` |
| macOS 非绝对静态链接 | macOS 上只承诺单文件、无随包动态库、仅系统框架（不做"绝对零动态依赖"承诺，§5.3） | 设计文档 §5.3 |
| UI 本地化基础层已落地（深度翻译后续迭代） | 🟡 部分：**已落地**（commit 8e8ac6f）——`ui/src/i18n.rs` 新增 `strings_catalog!` 宏（单一来源：字段声明 + 英文构造器 + 完整性测试表）、`Strings` 81 键、`Lang` 枚举（含 `En`）与 `Lang::strings` 选择入口、`const L`（`i18n.rs:34-63, 65-235, 237-259`）；lib.rs 导航/角色/公共操作/状态/错误/字段文案全部改 `{L.x}` 引用（`mod i18n;` `ui/src/lib.rs:45`）；ui 127 测试全过（含 3 个 i18n 测试，`i18n.rs:268-308`）、clippy/fmt/wasm32 干净；`web/assets/rutilus_ui.js/.wasm` 再生成、二次生成字节一致（commit 53b6402）。**未做（后续迭代）**：深度翻译（视图标题/空态/不可用消息/`CommandFamilyView` 标签/facts 词汇表/健康词汇/`OEM_UNSUPPORTED_NOTICE` 等）、语言选择器（`Translations` trait 刻意推迟，`i18n.rs:19-21`）。审计（I1）3 条 MINOR：`i18n.rs:1` 头注释 §5.1 引用不可核验（设计文档无「本地化/i18n」条目）、`L.action_delete`/`L.field_role` 语义复用、`aria-label="Loading"` 未抽取（`ui/src/lib.rs:11729`）；后续项登记见 `milestone-status.md` §7.2-A「UI 本地化」行 | `ui/src/i18n.rs`；`ui/src/lib.rs`；`web/assets/` |
