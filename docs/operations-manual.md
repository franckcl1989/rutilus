...
| Secret 泄漏扫描门禁 | **已启用**（2026-08-12，E3b）：`security/tests/secret_leak_gate.rs`——3 规则（R1 硬编码秘密 / R2 内嵌私钥 PEM 块 / R3 明文输出宏泄露）、9 测试（wave-one 73d480d 补间接赋值盲区后，2026-08-13 实测）、`ALLOWED_CONSTANT_HITS` 白名单 2 处（path+line+name+literal 绑定，`app/src/backup.rs:88, 89`）、`test-support` crate 目录级豁免（fixture scope，E3b 原始提交 eefde7e，`secret_leak_gate.rs:96-101, 1258`；深度审查批次 e8424df 另补 `strings_catalog!` 宏体结构豁免——CATALOG_MACRO 帧识别 + 新测试 `strings_catalog_macro_bodies_are_copy_construction_not_secret_assignments` `:1521`）；**CI 独立步骤**（`ci.yml:282-284` Secret leak gate：`bash scripts/assert-tests-ran.sh 8 --locked -p rutilus-security --test secret_leak_gate`，`if: matrix.is_default`，machete 之后、wasm32 之前，W6-1 ran-断言 floor 8，header 注记 `ci.yml:15-17`） |
