...
>
> 修订说明：本版为 **H4/H5 合入后复核版**（HEAD = d77d54e）。迭代六（H4/H5）已合入：**UI
> 本地化完整落地**（H5 d3f7769：`strings_catalog!` 目录扩至 827 键 En/Zh 双语、`Lang::{En, Zh}`
> 运行时语言选择器、URL fragment 持久化（`#lang=`，`ui/src/lib.rs:11599-11636`）；0f91c17
> `web/assets/rutilus_ui.js/.wasm` 再生成）与**发布管道代码侧**（H4 34503ea + d77d54e：
> `scripts/` 5 脚本 + ci.yml `release-artifacts` job，证书到位即启用）。§三-A「UI 本地化」行已
> 转 ✅、条件 17 已转 🟡 代码侧完成、§四-3 差距清单同步修订；此前各轮合并项（E1/E3a/E3b/E3c/
> E4、H1/H2）行号已逐条重核为当前 master 实际值（见 §五、§六）。门禁复跑全绿：migration 30 /
> persistence 190+3 / application 293 / infra 291 / web 全过 / **ui 136**（本轮复跑，127→136，
> 含 11 个 i18n 测试）/ rutilus 141 / security 门禁 7，clippy 全 crate 零警告、fmt 干净
> （2026-08-12）。
...
| 跨平台 E2E | ✅ | `ci.yml:130-146`（windows/macos 任务，web/tests 9 个路径套件 + `app/tests/version.rs`） |
...
| Secret 泄漏检查 | ✅ | 结构性防护（`milestone-status.md:203`）+ 独立扫描门禁已落地（E3b：`security/tests/secret_leak_gate.rs`，3 规则 R1/R2/R3、7 测试；CI 独立步骤 `ci.yml:224-226` Secret leak gate，`cargo test --locked -p rutilus-security --test secret_leak_gate`，machete 之后、wasm32 之前，`if: matrix.is_default`，header 注记 `ci.yml:15-17`；运行时抓包/日志复核仍为 §四-B 演练项） |
...
| 7 | 三平台单二进制发布 | 🟡 | 结构性（构建矩阵）+ 实测缺位 | 构建矩阵入 CI：x86_64 musl（`ci.yml:253-259`）、aarch64 musl cargo-zigbuild（`ci.yml:265-270`）、macOS Universal 2 lipo 合并 + `lipo -verify_arch x86_64 arm64` 校验（`ci.yml:288-304`）；三平台编译 + wasm32 UI 产物 diff（`ci.yml:65-81, 233-244`）；Windows ARM64 明确不入 CI（`ci.yml:271-278` 注释：hosted x64 runner 无 ARM64 MSVC 链接器与 SDK 导入库）；发布配置与 §5.4 一致（`Cargo.toml:110-116`；`rust-toolchain.toml` 已固定；Cargo.lock 已提交）；单二进制自包含边界（`support-matrix.md:85-88`） | ① Windows ARM64 发布目标无 CI 构建、无安装验证（§四-B，前置：原生 ARM64 runner 或本地 ARM64 主机）；② 三平台**发布包级**安装/运行验证并入条件 15 演练（§四-B）；③ 签名（条件 17）前置 |
...
| 发布级 CI 扩展（Windows ARM64 原生 runner） | ✅ 移交 §三-B（依赖原生 runner，非 A 类可做） | — | 原生 ARM64 Windows runner 或本地 ARM64 主机验证后另行处理 | `ci.yml:271-278` 注释；§三-B「Windows ARM64 发布验证」行 |
...
- 本版（HEAD = d77d54e）已登记迭代六（H4/H5）落地：UI 本地化完整落地（d3f7769 + 0f91c17）
  与发布管道代码侧（34503ea + d77d54e）。迭代六新增触面（`ui/src/i18n.rs`、`ui/src/lib.rs`、
  `scripts/`、ci.yml `release-artifacts` job）的每个 file:line 均在本轮打开文件核实
  （§六记录）；此前的 E1/E3a/E3b/E3c/E4、H1/H2 触面行号保持合并后已核实的值。
- 门禁复跑（2026-08-12，HEAD d77d54e）：`ci.yml:306-330`（Migration `:306-310`、Capability
  Ledger `:312-317`、Release Baseline `:319-330`）与全 workspace 测试全绿（migration 30 /
  persistence 190+3 / application 293 / infra 291 / web 全过 / **ui 136**（本轮复跑）/
  rutilus 141 / security 门禁 7），clippy 全 crate 零警告、fmt 干净。
- 引用自检记录见下节（每个 file:line 均在本轮打开核实）。

## 六、引用自检记录（2026-08-12，HEAD d77d54e 复核）
...
| `ci.yml`：`:3-24`（门禁清单注释，含 secret-leak gate `:15-17`）、`:53-64`（RUTILUS_GIT_COMMIT job 级注入，E3a）、`:71-80`（三平台矩阵）、`:120-122`（全 workspace Test）、`:130-146`（跨平台 E2E）、`:196-204`（cargo audit）、`:224-226`（Secret leak gate 独立步骤，E3b/G1）、`:233-244`（wasm32 UI 产物 diff）、`:253-259`（musl x86_64）、`:265-270`（aarch64 zigbuild）、`:271-278`（Windows ARM64 不入 CI 注释）、`:288-304`（Universal 2 + lipo verify_arch）、`:306-308`（Migration 门禁）、`:312-317`（Capability Ledger）、`:319-330`（Release Baseline） | 打开核实（d77d54e 重排 `on:` 块（新增 `v*` tag / `workflow_dispatch`，`:28-40`）与门禁段注释，旧 ci.yml 行号漂移 **+1~+8 不等**：RUTILUS_GIT_COMMIT 注入 45-55→53-64、三平台矩阵 63-72→65-81、跨平台 E2E 122-138→130-146、cargo audit 188-196→196-204、Secret leak gate 216-218→224-226、wasm32 产物 diff 225-235→233-244、musl 245-250→253-259、zigbuild 257-261→265-270、ARM64 注释 263-270→271-278、Universal 2 280-295→288-304、Migration 299-301→306-310、Capability Ledger 306-308→312-317、Release Baseline 318-320→319-330；本行已按新行号引用，其余文档内旧 ci.yml 引用未逐处改写，待下轮统一复核换算） |
