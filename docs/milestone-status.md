...
> 触面 auth.rs/web lib.rs/ci.yml 等按当前值重锚；§7.4/§7.5 历史批次的登记保留其当时 HEAD 下的事实）：
...
| 数据库 Schema | 21 个 migration（`m20260805_*` 11 + `m20260807_*` 8 + `m20260810_*` 2）；迭代三/四新增 2 个（`m20260812_000001_resource_decode_failures`、`m20260812_000002_resource_feature_lists`），迭代十五（wave-one）新增 2 个（`m20260813_000001_audit_center_actions`、`m20260813_000002_endpoint_health_checks`），现共 **25 个** | `migration/src/`（迁移测试 `migration/tests/initial_storage.rs`、`migration/tests/resource_feature_lists.rs`、`migration/tests/audit_center_actions.rs`、`migration/tests/endpoint_health_checks.rs`） |
...
中心重连风暴测试（`center_sync.rs` 并发测试）、跨平台 E2E 运行（`ci.yml:146-162`）、
...
| Center/Site 长时间断线重连演练 | 0.9.0 验收项；并发重连风暴已自动化覆盖（`center_sync.rs:4447, 4567, 4734, 4957, 5087`），长时间（跨进程/跨天）真实断线演练仍未执行 | 设计文档 §0.9.0 验收 |
...
| `8147bc9` | 认证边界硬化（B1 密码策略 / B2 429 审计 / B3 撤销信号 / B4 哑验证补齐） | `web/src/auth.rs:1488-1489, 1519, 1540-1566, 1569-1604, 1951-1962`（wave-one d3b966a 重写 auth.rs 后重核） |
...
| `4897b22` | T-B：入网首刷改走 `endpoint_read_gate`，与批量刷新不再重叠；新增 `EndpointReadGateError` 导出 | `application/src/endpoint_enrollment.rs:158-202`（gate 获取 `:168-179`、`refresh.execute` `:190`）、对抗测试 `:643`；`application/src/lib.rs:85-86`；`web/src/lib.rs:3307`（错误映射） |
| `e7aef53` | T-D：限速器桶键 4096 阈值周期剪枝，内存有界 = 窗口活跃工作集 + 4096；web 全过（当时 133，wave-one 后 147） | `web/src/auth.rs:133`（`BUCKET_PRUNE_THRESHOLD`）、`:969-1160`（`LoginRateLimiter`/`BucketMap`/`prune_if_due` `:1106`/`prune_expired` `:1122`）；有界性测试 `:3238, 3306, 3350, 3385`（S3-3 原子 reserve/refund 后第三个测试更名为 `rate_limiter_prune_reclaims_compensated_empty_buckets`） |
