...
> 基于当前 master（commit c4168c5）的**代码级自查记录**。
>
...
| 级别 | 编号 | 问题 | 推理链 / 复现路径 |
|---|---|---|---|
| MINOR | M1 | 登录路径存在用户枚举时间侧信道 | `web/src/auth.rs:1241-1254`：用户名不存在时在 Argon2id 之前提前返回（仅哈希一次不存在的查询）；`:1277-1287`：口令错误时执行完整 Argon2id（64 MiB/3 轮，约数百 ms）。错误文案统一（"sign-in failed"），但耗时差异可区分「用户存在但口令错」与「用户不存在」。攻击者借助限速预算内（5 次/用户名/15 分钟）或分布多用户名探测可枚举有效账户名。缓解：未知用户名路径执行一次哑 Argon2id（对固定哈希）。审计与限速均已覆盖该路径（`record_login_failure` 在两种分支都调用），仅时间侧信道未消除。审查推断：严重度 MINOR——需要本地时序测量才能利用，且限速限制单用户名吞吐。 |
...
范围 + §7.7 扫描全部完成，**每一项都有真实代码行号证据**，无 BLOCKER，MINOR 1 项、NOTE 8 项
（见第三节）。
...
| 项 | 说明 | 依据 |
|---|---|---|
| 独立 Secret 泄漏扫描 | 结构性防护证据充分，但无独立扫描工具/演练（如 grep 级扫描、内存转储检查、API 响应抓包复核）。milestone-status §7.1「Secret 泄漏检查」同样标 🟡 部分 | `docs/milestone-status.md:191` |
| M1 时间侧信道处置 | 需决定是否在未知用户名路径补哑 Argon2id（一行级改动），或登记为 1.0.0 已知限制 | 本文档 M1 |
...
- 附加条件：该结论当前基于**代码审查与结构证据**，不是基于独立泄漏扫描或外部评估；
  M1 时序侧信道不构成「凭据泄漏」（不泄露密码/哈希，只泄露账户存在性），但应作为已知项登记。
  建议 1.0.0 发布评审时补充：独立 Secret 泄漏扫描（仓库级 + 运行时抓包/日志复核）、
  一次外部安全评估（含 §7.7 合规、Pin 验证器、备份包格式），并将本文档的 MINOR/NOTE
  逐条关闭或转登记为已知限制。
