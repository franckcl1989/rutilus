...
| HIGH | R6-C-1 | 并发双派发铸双 id 双执行——**已修复**（wave-six，fcf7257） | 原状：dispatch 幂等为 check-then-act——两个并发相同派发各铸一个 operation id，站点对同一 BMC 写执行两次（§13.5 禁止双执行，且两操作各自独立终态、审计双记）。**修复（commit fcf7257）**：`StandaloneState` per-site `dispatch_gates`（`app/src/standalone_runtime.rs:307-313`），`dispatch_center_operation` 把绑定检查→find_undecided→create_operation→enqueue_offer 整体纳入同一临界区（`app/src/center_runtime.rs:472-515`，锁仅在 dispatch 内持有、无重入路径）。测试 `two_concurrent_identical_dispatches_create_one_operation`（`center_runtime.rs:1566`）钉死同一 operation id + 恰 1 条非终态操作。登记见 `known-limitations.md` §九（第六波块）。 |
| HIGH | R6-E-01 | Unknown 后重派发铸新 id 逃过 inbox 去重——**已修复**（wave-six，fcf7257） | 原状：Unknown 结局（响应丢失）后重派发铸新 id，站点 inbox 按 operation_id 查重看不见它，同一写被双执行（§13.5 禁止）。**修复（commit fcf7257）**：`CenterDispatchError::UnknownOutcomePending { operation_id }`（`application/src/center/dispatch.rs:283-300`）——find_undecided 无候选后检查同 (site, endpoint, command) 的终态 Unknown 操作（经 outbox 定向行确认，防 re-home 误伤），存在则类型化拒绝且不铸新 id（`:389-397, 655-697`）；web 映射 409 + 稳定码 `unknown_outcome_pending`（7c6ac9d 接线）。测试 `a_retry_after_an_unknown_outcome_is_refused_with_the_existing_operation_id`（`dispatch.rs:3978`）。登记见 `known-limitations.md` §九（第六波块）。 |
| MEDIUM | R6-S-1 | 会话撤销 fail-open——**已修复**（wave-six，fcf7257） | 原状：撤销路径把存储失败答成 404 且不审计，会话实际保持有效（撤销请求方以为已撤销——fail-open）。**修复（commit fcf7257）**：`SessionRevocation::{Revoked, NotFound, AlreadyRevoked}` 三态边界（`web/src/auth.rs:339, 370`）——Revoked/AlreadyRevoked→200 + ManageSessions 审计（幂等成功）、NotFound→404 不审计、存储类失败→500 + 失败审计事件（仿 B3 纪律，`auth.rs:2617-2660`；运行时分类映射 `app/src/standalone_runtime.rs:801-820`）；logout 的撤销失败仍 500。测试 `revoke_session_surfaces_a_storage_failure_as_500_with_audit`（`web/src/lib.rs:12817`）等三枚。登记见 `known-limitations.md` §九（第六波块）。 |
| MEDIUM | R6-S-3 | secret-gate 标识符集缺口——**已修复**（wave-six，fcf7257） | 原状：`SENSITIVE_IDENTIFIERS` 缺 `binding_code`（站点绑定码未受 [R1] 保护），复合后缀集无 `_pwd/_passwd/_pw/_passcode`。**修复（commit fcf7257）**：补 `binding_code` 与四个无条件后缀；`_key/_pin` 按误报张力单独登记（`TENSION_IDENTIFIER_SUFFIXES`，仅绑定非空字面量触发 [R1]、不进 [R3] 日志集）；`_key` 后缀新增命中的 5 个生产常量进 `ALLOWED_CONSTANT_HITS`（2→7 条，path+line+name+value 绑定，漂移即红）（`security/tests/secret_leak_gate.rs:517-601`）。测试 `binding_code_and_new_compound_suffixes_are_flagged`（`:1853`）。登记见 `known-limitations.md` §九（第六波块）。 |
| LOW/NOTE | R6-S-4/S-5/S-6/S-7/A4-B1/S-11 | secret-gate 与 master-key 批次——**已修复**（wave-six，fcf7257） | ① PEM 转义击穿：`parse_plain_string` 完整转义解码（`\xNN`/`\u{...}`/续行），content 与编译值一致（`secret_leak_gate.rs:306-339`）；② `print!`/`eprint!` 入输出宏集（3→5，`:654`）；③ const/static 链与方法调用形态盲区如实登记（头文档 `:43-50`）；④ concat! 片段规则改跨片段三元组判定（BEGIN+END+PRIVATE KEY 三片齐备即报 [R2]，不论绑定名，`:1132`）；⑤ tokenizer 两机械缺陷（`b'x'` 字节字面量消费 + 多行字面量行号补偿，报点行号恢复真实源行，`:273-339, 497`）；⑥ master-key KDF 对齐产品基线（64 MiB/3 趟/1 并行），信封魔数升版 `RUTMK001→RUTMK002`，旧版信封仍可解锁且解锁后经 `rewrap_master_key` 迁移重保护落盘（`security/src/master_key.rs:16-21, 225-250, 447-497`；`platform/src/master_key_file.rs:97`）。门禁现 15 测试。登记见 `known-limitations.md` §九（第六波块）。 |
| LOW/NOTE | R6-S-2/S-8/S-9/S-10/S-13/W-4/W-9/R6-C-5 | 认证/限速面批次——**已修复**（wave-six，fcf7257） | 双重 Set-Cookie（清除头不再被续期头覆盖，`web/src/auth.rs:1494-1508`）、bootstrap 未知主体补审计（`:2136-2146`）、用户名限速键归一化投影（`:1404-1427`）、admin 404 分支挂 password-change 同形预算（`:2770-2810, 2875-2915`）、`me` 的 bootstrap 闸读失败倒向「claim 待定」（`:2509-2519`）、IPv4-mapped 归一化防双桶（`:1682-1698`）、`me` per-IP 查询限速（`:143-147, 1041-1056, 2488-2519`）、refund token 精确化（PARTIAL，`:1134-1364`）。登记见 `known-limitations.md` §九（第六波块）。 |
| LOW | R6-W-3 | 伪造 center 归因——**已修复**（wave-six，7c6ac9d） | 原状：HTTP 控制台面信任客户端提交的 `source: "center"`，伪造归因可把本地提交伪装成中心派发。**修复（commit 7c6ac9d）**：`create_operation` 服务端 pin——只接受 `standalone`/`site`（按姿态），`center` 保留给中心 offer 流，wire 串 400 拒绝且不落库（`web/src/lib.rs:2312-2323`）；只读投影如实不变（`:2923-2927`）。测试 `accepts_an_explicit_operation_source`（`web/tests/operation_path.rs:1168`）；inbox 污染半边 refuted。登记见 `known-limitations.md` §九（第六波块）。 |
...
| 项 | 说明 | 依据 |
|---|---|---|
| 独立 Secret 泄漏扫描 | ✅ 仓库级已落地（迭代三，E3b）：`security/tests/secret_leak_gate.rs`（3 规则 R1/R2/R3、**15 测试（V4I-3 重测 10、wave-six 增补 5）**、`ALLOWED_CONSTANT_HITS` 白名单 2 处、`test-support` crate 目录级豁免（E3b 原始提交 eefde7e）`:96-101, 1258`、深度审查批次 e8424df 补 `strings_catalog!` 宏体豁免 `:575, 1038-1043, 1521`、wave-one（73d480d）补间接赋值盲区 `:836`、wave-two（e59b14a）补跨字面量 PEM 片段盲区 `:886`、全 workspace 扫描全绿），**CI 独立步骤**（`ci.yml:322-324` Secret leak gate：`bash scripts/assert-tests-ran.sh 10 --locked -p rutilus-security --test secret_leak_gate`，`if: matrix.is_default`，machete 之后、wasm32 之前，W6-1 ran-断言 floor 10）；**运行时复核未做**：内存转储检查、API 响应抓包/日志复核仍为 1.0.0 发布评审建议项。milestone-status §7.1「Secret 泄漏检查」已转 ✅ 结构性 | `docs/milestone-status.md:568` |
...
- 已满足的支撑面：BMC 凭据 at-rest 加密（XChaCha20-Poly1305 + AD 绑定 + 随机 Nonce，
  ...
  （`application/src/center_sync.rs:1796`）；命令列与中心队列 payload at-rest 加密
